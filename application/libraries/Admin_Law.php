...
class Admin_Law extends BaseRow
{
	...
	public function get_all($limit=50,$st=0,$order='id',$order_type='desc',$where=array()) {	
		...
		$result=$this->CI->db->query("SELECT    u.* FROM users u, user_type t WHERE t.id=u.user_type_id AND t.admin=1 ")->result_array(); 
		return $result;
    }
	...
	public function generate_form_rows($class='')
	{
		...
		foreach ($res as $row) $admin_users[$row['id']]=$row['email'];
