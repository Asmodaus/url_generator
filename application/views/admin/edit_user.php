...
include('header2.php');
$row_sum=$this->db->query("SELECT   SUM(sum) sum,   COUNT(id) count FROM exchange WHERE user_id='{$model->id}' ")->row_array();
        ...
        <section class="card">
            <div class="card-block">
                ...
                <div class="btn-group-vertical btn-group-justified">
					...
					<?if ($model->webpush==1):?>
					<button type="button" class="btn" data-toggle="modal" data-target="#send-message2">Отправить WebPush</button>
					<?endif;?>
					<?if ($model->ban==0):?>
                    <button type="button" class="btn" data-toggle="modal" data-target="#ban-user">Забанить</button>
					<?else:?>
					<button type="button" class="btn"  OnClick="ajax('/ban/','id=<?=$model->id?>&ban=0' ,'1');">Разбанить</button> 
					<?endif;?>
                    */ ?>
        ...
        <section class="card">
            <div class="card-block">
                ...
                <dl class="row">
                    ...
                    <dd class="col-xl-8"><h3><span class="badge badge-default text-truncate" style="width:100%;"><?=$Country->name?></span></h3></dd>
                    <dt class="col-xl-4">Город:</dt>
                    <dd class="col-xl-8"><h3><span class="badge badge-default text-truncate text-justify" style="width:100%;"><?=$model->city?></span></h3></dd>
        ...
        <section class="card">
            <div class="card-block">
                ...
                <dl class="row">
					...
					<dt class="col-xl-3">BCH:</dt>
                    <dd class="col-xl-9"><p class="font-size-12"><a href="javascript: void(0);" target="_blank" class="cat__core__link--underlined"><?=$model->cash8?></a></p></dd>
				...
				<form action="javascript:" method="post">
                    <table class="table thead-default table-hover nowrap" id="example1" width="100%">
                        ...
                        <tbody>
							...
							<?
							$s1=$s2=$s3=0;
							 foreach ($this->db->query("SELECT   e.* FROM exchange e  WHERE create_time>='".strtotime($_GET['time1'])."' AND  create_time<'".(strtotime($_GET['time2'])+24*3600)."'  AND e.user_id='{$model->id}' ORDER BY e.id DESC ")->result_array() as $row):
							 $pay_methods=row('valut',$row['from']);
							 $valut=row('valut',$row['to']);
							 $s1+=$row['sum'];
							 $s2+=$row['sum_to']; 
							  $s3+=$row['profit']; 
							 ?>
                            ...
                            <td><?=$row['sum_to']?></td>
                            <td>€<a href="/admin55/page/exchange?id=<?=$row['id']?>" target="_blank"><?=$row['sum']?></a></td>
                            <td><?=$row['profit']?></td> 
                            <td><?=$row['kurs']?></td> 
                            <td><a href="https://blockchain.info/address/<?=$row['wallet']?>" target="_blank" class="cat__core__link--underlined"><?=$row['wallet']?></a></td>
                            <?/*
                            <td>
...
<div class="modal fade" id="photopass2" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel"><a href="/upload/<?=$model->passport2?>">Скачать</a></h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <img src="/upload/<?=$model->passport2?>" class="img-fluid" />
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Закрыть</button>
      </div>
...
<div class="modal fade" id="adresdoc" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel"><a href="/upload/<?=$model->adresdoc?>">Скачать</a></h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">  
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <img src="/upload/<?=$model->adresdoc?><?=$model->bank?>" class="img-fluid" />  
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Закрыть</button>
      </div>
...
<div class="modal fade modal-size-large" id="send-message2" tabindex="-1" role="dialog" aria-labelledby="" aria-hidden="true">
    <div class="modal-dialog" role="document">
		<form   class="form-horizontal" action="javascript:void(null);" method="post" OnSubmit="ajax_post('send_push/<?=$model->id?>',this,'#resmail2');">
      
        <div class="modal-content">
            <div class="modal-header">
                <h4>Отправить Web Push <?=$model->name?> <?=$model->suname?></h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
